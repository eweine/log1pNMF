...
i       <- which(samples$mouse == "S2")
samples <- samples[i,]
counts  <- counts[i,]

outliers <- c("TTTGTTGGTAGCTGTT-1","CATCCGTAGGCCTAAG-1", "AGGGTTTGTTGGGTTT-1")
...
readr::write_csv(celltype_df, "~/Downloads/pancreas_cytokine_S2_celltypes.csv")
