...
set.seed(1)
log1p_mod_k1 <- fit_factor_model_log1p_quad_approx_sparse(
  Y = counts, K = 1, s = s, approx_range = c(0, 1.25), maxiter = 5
)

# Now, I want to initialize with k=1
U_init <- cbind(
  log1p_mod_k1$U,
  matrix(
    data = rexp(n = nrow(log1p_mod_k1$U) * 29, rate = 15), nrow = nrow(log1p_mod_k1$U)
  )
)
...
V_init <- cbind(
  log1p_mod_k1$V,
  matrix(
    data = rexp(n = nrow(log1p_mod_k1$V) * 29, rate = 15), nrow = nrow(log1p_mod_k1$V)
  )
)

log1p_mod_k30_k1_init <- fit_factor_model_log1p_quad_approx_sparse(
  Y = counts, K = 30, s = s, approx_range = c(0, 1.25), maxiter = 100,
  init_U = U_init, init_V = V_init
)

readr::write_rds(log1p_mod_k30_k1_init, "~/Documents/data/passPCA/experiment_results/mouse_k30_k1_init.rds")

rownames(log1p_mod_k30_k1_init$U) <- rownames(counts)
rownames(log1p_mod_k30_k1_init$V) <- colnames(counts)
...
# initializations
max_col <- apply(log1p_mod_k30_k1_init$U, 2, max)
log1p_LL <- sweep(log1p_mod_k30_k1_init$U, 2, max_col, FUN = "/")
log1p_FF <- sweep(log1p_mod_k30_k1_init$V, 2, max_col, FUN = "*")
...
# I think the other two models that would be
# good to run here would be
# nmf on the log-transformed counts
# and the Poisson NMF model that peter implemented
# I think that I can put all of these into an analysis
# and check out how things look
