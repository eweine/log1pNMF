...
set.seed(1)
pois_nmf_mod <- fastTopics::fit_poisson_nmf(
  X = counts, k = 30, init.method = "random", control = list(nc=7)
)

readr::write_rds(pois_nmf_mod, "~/Documents/data/passPCA/experiment_results/pois_nmf_mouse_k30.rds")

max_col <- apply(pois_nmf_mod$L, 2, max)
log1p_LL <- sweep(pois_nmf_mod$L, 2, max_col, FUN = "/")
log1p_FF <- sweep(pois_nmf_mod$F, 2, max_col, FUN = "*")
