...
arma::vec solve_pois_reg_log1p_add_const (
    const arma::mat X,
    const arma::vec y,
  ...
  vec eta = X * b;
  vec exp_eta = exp(eta);
  vec exp_eta_nz_m1 = exp_eta.elem(y_nz_idx) - c;
  vec eta_proposed;
  vec exp_deriv_term;
  double t;
  double f_proposed;
  ...
  for (int update_num = 1; update_num <= num_iter; update_num++) {
    ...
    for (i = 0; i < num_indices; i++) {
      ...
      while (true) {
        ...
        exp_eta_nz_m1 = exp_eta.elem(y_nz_idx) - c;
...
arma::mat regress_cols_of_Y_on_X_log1p_pois_exact_add_const(
    const arma::mat X,
  ...
  for (int j = 0; j < B.n_cols; j++) {
    ...
    B.col(j) = solve_pois_reg_log1p_add_const (
      X,
      Y[j],
      Y_nz_idx[j],
      c,
...
// [[Rcpp::export]]
List fit_factor_model_log1p_exact_add_const_cpp_src(
  ...
  // TODO: change this likelihood calculation
  double loglik = get_loglik_exact_add_const(
    U_T,
    V_T,
    sc_x,
    sc_i,
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    U_T = regress_cols_of_Y_on_X_log1p_pois_exact_add_const(
      V_T.t(),
      y_rows_data,
      y_rows_idx,
      c,
    ...
    V_T = regress_cols_of_Y_on_X_log1p_pois_exact_add_const(
      U_T.t(),
      y_cols_data,
      y_cols_idx,
      c,
    ...
    loglik = get_loglik_exact_add_const(
      U_T,
      V_T,
      sc_x,
      sc_i,
