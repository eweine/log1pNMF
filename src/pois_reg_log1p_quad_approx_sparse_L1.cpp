...
arma::vec solve_pois_reg_log1p_quad_approx_sparse_L1_scalar_s (
    const arma::mat X_T,
    const arma::vec y,
  ...
  for (int update_num = 1; update_num <= num_iter; update_num++) {
    ...
    for (i = 0; i < num_indices; i++) {
      ...
      ) + X_0_cs_times_sa1[j] + 2 * a2 * s * dot_b + L1;
...
arma::vec solve_pois_reg_log1p_quad_approx_sparse_L1_vec_s (
    const arma::mat X_T,
    const arma::vec y,
    const arma::uvec y_nz_idx,
    const arma::vec s_nz,
  ...
  for (int update_num = 1; update_num <= num_iter; update_num++) {
    ...
    for (i = 0; i < num_indices; i++) {
      ...
      first_deriv    = dot(s_nz, exp_deriv_term_nz) - dot(
        y,
        exp_deriv_term_nz / exp_eta_nz_m1
      ) + X_T_0_s_times_a1[j] + 2 * a2 * dot_b + L1;
...
arma::mat regress_cols_of_Y_on_X_log1p_quad_approx_sparse_L1_vec_s(
    const arma::mat X_T,
  ...
  for (int j = 0; j < B.n_cols; j++) {
    ...
    B.col(j) = solve_pois_reg_log1p_quad_approx_sparse_L1_vec_s(
      X_T,
      Y[j],
      Y_nz_idx[j],
      s.elem(Y_nz_idx[j]),
...
arma::mat regress_cols_of_Y_on_X_log1p_quad_approx_sparse_L1_scalar_s(
    const arma::mat X_T,
    const std::vector<arma::vec> Y,
    const std::vector<arma::uvec> Y_nz_idx,
    const arma::vec s,
  ...
  for (int j = 0; j < B.n_cols; j++) {
    ...
    B.col(j) = solve_pois_reg_log1p_quad_approx_sparse_L1_scalar_s(
      X_T,
      Y[j],
      Y_nz_idx[j],
      s(j),
...
// [[Rcpp::export]]
List fit_factor_model_log1p_quad_approx_sparse_L1_cpp_src(
  ...
  for (int iter = 0; iter < max_iter; iter++) {
    ...
    V_T = regress_cols_of_Y_on_X_log1p_quad_approx_sparse_L1_vec_s(
      U_T,
      y_cols_data,
      y_cols_idx,
      s,
    ...
    U_T = regress_cols_of_Y_on_X_log1p_quad_approx_sparse_L1_scalar_s(
      V_T,
      y_rows_data,
      y_rows_idx,
      s,
  ...
  arma::vec d1 = arma::max(U_T, 1);
  arma::vec d2 = arma::max(V_T, 1);
