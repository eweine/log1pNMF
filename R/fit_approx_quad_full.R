#' Fit full quadratic approximation log1p factor model
#'
...
fit_factor_model_log1p_quad_approx_full <- function(
  ...
  for (iter in 1:maxiter) {
    ...
    new_U_T <- regress_cols_of_Y_on_X_log1p_quad_approx_full(
      t(fit$V),
    ...
    new_V_T <- regress_cols_of_Y_on_X_log1p_quad_approx_full(
      t(fit$U),
...
#' Fit full quadratic approximation log1p factor model with C++
#'
#' @param Y sparse matrix.
#' @param K rank of factorization
#' @param approx_range range of Chebyschev approximation
...
fit_factor_model_log1p_quad_approx_full_cpp <- function(
  ...
  fit <- fit_factor_model_log1p_quad_approx_full_cpp_src(
    sc$x,
    sc$i - 1,
    sc$j - 1,
    sc_t$x,
