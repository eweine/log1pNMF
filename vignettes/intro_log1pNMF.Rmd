---
title: Using log1p NMF to Analyze scRNA-seq Data
author: Eric Weine, Peter Carbonetto, Rafael Irizarry, and Matthew Stephens
date: "`r Sys.Date()`"
output:
  html_document:
    toc: no
    ...
    theme: readable
vignette: >
  %\VignetteIndexEntry{Using log1p NMF to Analyze scRNA-seq Data}
...
The aim of this vignette is to demonstrate how to fit a log1p Poisson NMF model and analyze its output using the `log1pNMF` package. 
...
For an $n \times p$ matrix of counts $\mathbf{Y}$, a log1p NMF model is defined as
\begin{align}
y_{ij} &\sim \textrm{Poisson}(\lambda_{ij}) \\ 
g(\lambda_{ij}; c) &= b_{ij}  \\ 
\mathbf{B} &= \mathbf{L}\mathbf{F}^{\top},
\end{align}
where $\mathbf{L}$ and $\mathbf{F}$ are non-negative $n \times K$ and $p \times K$ matrices, respectively, and $c > 0$ is a hyper-parameter which controls the behavior of the "shifted-log link function", defined as 
\begin{equation}
g(\lambda; c) = \max(1, c) \cdot \log\left(1 + \lambda/c\right).
\end{equation}

For more details / intuition about the log1p NMF model, see [Weine *et al* 2026][weine-2026].

An example using scRNA-seq
--------------------

To fit a log1p NMF model, the user has to (at minimum) provide 3 inputs:

1. A matrix of counts $\mathbf{Y}$.
2. A setting of $K$ to determine the rank of the factorization.
3. A setting of $c$ to determine the behavior of the shifted-log link function.

As an example count matrix, we will use a scRNA-seq data set from [Zheng *et al* (2017)][zheng-2017]. These data are reference
...
Now we have a 3,774 x 3,000 counts matrix:

```{r subset-genes-2}
dim(counts)
```

The original dataset contains cell type labels for each cell. For convenience, we will combine all the T cell labels into one
...
```
```{r}
log1p_fit_approx <- fit_poisson_log1p_nmf(
  Y = counts,
  K = 6,
  cc = 1,
  loglik = "approx",
...
```{r}
tm_fit <- fit_poisson_nmf(X = counts, k = 5)
```
...
```
```{r}
log1p_fit_smaller <- fit_poisson_log1p_nmf(
  Y = counts,
  K = 5,
  cc = 1,
  loglik = "exact",
...
[weine-2026]: https://arxiv.org/abs/2601.05845
